...
class Node:
    ...
    """
        估价公式：欧拉距离算法
     """
    def oular(self,endNode):
        self.h = ((endNode.point.x - self.point.x)**2 + (endNode.point.y - self.point.y)**2)*10
    def setG(self, g):
        self.g = g
...
class Axing:
    ...
    def searchOneNode(self,node):
        ...
        if self.nodeInOpenlist(node) == False:
            node.setG(gTemp)
            #H值计算
            node.oular(self.endNode);
            node.father = self.currentNode
            # node.setG(self.currentNode.g+11)
    ...
    def searchNear(self):
        ...
        (x+1,y-1)(x+1,y)(x+1,y+1)
        """
        if self.map2d.isPass(Point(self.currentNode.point.x - 1, self.currentNode.point.y)) or \
        ...
        if self.map2d.isPass(Point(self.currentNode.point.x, self.currentNode.point.y - 1)) or \
    ...
    def start(self):
        ...
        '''
        #将初始节点加入开放列表
        self.startNode.oular(self.endNode);
        ...
        while True:
            ...
            # print('x= ',self.currentNode.point.x,' y= ',self.currentNode.point.y)
    ...
    def setMap(self):
        for node in self.pathlist:
            ...
            if  self.map2d.isPass(Point(node.point.x+1,node.point.y))==False and self.map2d.isPass(Point(node.point.x-1,node.point.y))==False or\
                self.map2d.isPass(Point(node.point.x,node.point.y+1))==False and self.map2d.isPass(Point(node.point.x,node.point.y-1))==False:
                self.uselist.append(node)

        firstNode = self.startNode
        self.uselist=self.uselist[::-1]
        for node in self.uselist:
            if firstNode.point.x-node.point.x !=0:
                k=(firstNode.point.y-node.point.y)/(firstNode.point.x-node.point.x)
                b=firstNode.point.y-k*firstNode.point.x
                for x in range(firstNode.point.x,node.point.x+1):
                    y=int(k*x+b)
                    N=Point(x,y)
                    self.map2d.setMap(N)

                for y in range(firstNode.point.y, node.point.y + 1):
                    x=int((y-b)/k)
                    N = Point(x, y)
                    self.map2d.setMap(N)
                firstNode = node
